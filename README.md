...
3. Click the extension icon → ⚙️ Settings → select **OpenAI GPT-4o Mini** → paste key → Save
...
3. Click **Summarize** (or choose **3 bullets** mode for a shorter summary)
...
- Calls Gemini (`gemini-2.5-flash` via `generativelanguage.googleapis.com`) or OpenAI (`gpt-4o-mini` via `api.openai.com`)
...
The AI prompt requests a structured JSON response:
...
Temperature is set to `0.3` for consistent, factual summaries. The response is parsed with `JSON.parse` and falls back to heuristic line-splitting if the model returns malformed output.

Gemini uses the current stable Flash model, `gemini-2.5-flash`. Older `gemini-1.5-*` model names are retired and can return `model not found` errors.
